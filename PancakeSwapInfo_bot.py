...
def start_command(message):
    ...
    bot.send_message(cid, "🔰 გამარჯობა 🥳 მე PancakeSwap-ის ბოტი ვარ და შემიძლია შენი მისამართით გაგიგო რომელ Pool-ში რამდენი ტოკენი დააგროვე და ასევე " + \
                          ...
                          "🔰 ჯერ მხოლოდ 9 Pool-ზე შემიძლია ინფორმაცია მოგაწოდო: *CAKE, DITTO, HGET, BDO, EGLD, UST, WSOTE, FRON, HELMET, BTCST, BSCX, LINA, SFP, ZEE და BRY.*" + "\n" + "\n" + \
...
def help_command(message):
    cid = message.chat.id
    bot.send_message(cid, "🔰 თუ გინდა იცოდე რამდენი დააგროვე *CAKE, DITTO, HGET, BDO, EGLD, UST, WSOTE, FRON, HELMET, BTCST, BSCX, LINA, SFP, ZEE და BRY* Pool-ში მაშინ უბრალოდ მისამართი მომწერე." + "\n" + "\n" + \
...
def contract_checker(guy):
    ...
    if check_if_zeros == True and cake_list[0] == 0:
        return "ამ მისამართით CAKE, DITTO, HGET, BDO, EGLD, UST, WSOTE, FRON, HELMET, BTCST, BSCX, LINA, SFP, ZEE და BRY Pool-ში CAKE არ არის დასტეიკებული!"
    else:
...
def send_pcs_info(message: Message):
    try:
        ...
        contracts = [cake, ditto, bdo, egld, ust, wsote, front, helmet, btcst, bscx, ten, albt, zee, bry]
        contract_names = ["CAKE", "DITTO", "BDO", "EGLD", "UST", "WSOTE", "FRONT", "HELMET", "BTCST", "BSCEX", "TEN", "ALBT", "ZEE", "BRY"]
        ...
        info = "🥞 1 CAKE = $" + '{0:,.3f}'.format(float(coin_fasi)) + "\n" + \
                "🔸 სრული რაოდენობა: " + '{0:,.2f}'.format(float(totalsupply)) + "\n" + \
                "🔸 დამწვარია: " + '{0:,.2f}'.format(float(burned)) + "\n" + \
                "🔸 დაიწვება: " + '{0:,.2f}'.format(float(mondeyBurn)) + " ($" + '{0:,.2f}'.format(float(mondey_burn_price)) + ")" + "\n" + \
                "🔸 დასტეიკებული: " + '{0:,.2f}'.format(float(total)) + " ($" + '{0:,.2f}'.format(float(total_price)) + ")" + "\n" + \
                "🔸 " + '{0:,.2f}'.format(float(staked_percent)) + "% სრული რაოდენობის დასტეიკებულია." + "\n" + \
                "🔸 ნავაჭრი (24h): $" + '{0:,.2f}'.format(float(trade_volume)) + "\n" + \
